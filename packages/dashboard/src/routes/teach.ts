...
import { type AgentToolContext, EXPLORATION_AGENT_TOOLS, executeAgentTool } from '../agentTools.js';
import {
  type AgentMessage,
  estimateTotalTokens,
  forceSummarize,
...
import type { createLlmProvider } from '../llm/index.js';
import type {
  ChatMessage,
  ChatWithToolsResult,
  StreamEvent,
...
import { type ProposeStepRequest, proposeStep } from '../teachMode.js';
...
function logFailedToolCall(
  ...
  try {
    ...
    console.warn(
      `[Agent] Failed tool call logged: ${entry.tool} [${entry.reason}] (conversation: ${entry.conversationId})`
    );
  } catch (err) {
    ...
    parameters: {
      ...
      properties: {
        id: {
          type: 'string',
          description:
            'Pack ID using dash-separated segments (e.g., "gmail-email-scraper", "yc-company-collector")',
        },
        name: {
          type: 'string',
...
export function createTeachRouter(ctx: DashboardContext): Router {
  ...
  router.post('/api/teach/propose-step', async (req: Request, res: Response) => {
    ...
    if (!ctx.llmProvider) {
      return res
        .status(503)
        .json({ error: 'LLM provider not configured (OPENAI_API_KEY required)' });
    }
    ...
    if (!request.packId || !request.userIntent || !request.elementFingerprint) {
      return res
        .status(400)
        .json({ error: 'packId, userIntent, and elementFingerprint are required' });
    }
  ...
  router.post('/api/teach/agent', async (req: Request, res: Response) => {
    ...
    const {
      messages,
      packId: requestPackId,
      conversationId,
      stream: streamFlowUpdates,
    } = req.body as {
    ...
    ): Promise<ChatWithToolsResult & { thinking?: string }> {
    ...
    try {
      for (let iter = 0; iter < MAX_TOTAL_ITERATIONS; iter++) {
        ...
        if (tokenEstimate > 100_000) {
          console.log(
            `[Agent] Token estimate ${tokenEstimate} exceeds threshold, attempting summarization...`
          );
          ...
          try {
            ...
            if (summaryResult.wasSummarized) {
              ...
              console.log(
                `[Agent] Summarized: ${summaryResult.tokensBefore} -> ${summaryResult.tokensAfter} tokens`
              );
              writeStreamLine({
                type: 'summarized',
        ...
        if (result.toolCalls && result.toolCalls.length > 0) {
          ...
          const hasNonEditorCall = result.toolCalls.some((tc: { name: string }) =>
            ...
            if (tc.name === 'request_secrets' && conversationId) {
              try {
                ...
                if (parsed?._type === 'secrets_request' && parsed.secrets && parsed.message) {
                  ...
                  console.log(
                    `[Agent] Waiting for user to provide secrets for conversation ${conversationId}...`
                  );
                    ...
                    setTimeout(
                      () => {
                        if (ctx.pendingSecretsRequests.has(conversationId)) {
                          ...
                          reject(
                            new Error(
                              'Timeout waiting for secrets - user did not respond within 5 minutes'
                            )
                          );
                        }
                      },
                ...
                resultStr = JSON.stringify({
                  error:
                    secretsError instanceof Error ? secretsError.message : String(secretsError),
                });
              }
            }
  ...
  router.post(
    '/api/teach/agent/:conversationId/secrets-filled',
